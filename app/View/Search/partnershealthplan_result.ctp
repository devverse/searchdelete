...
<tr>
	...
	<td>
		...
		<?php if($coor && $req_data['practicename'] == ''){?>
			<div><a data-map-type="coor" target="_blank" href="http://maps.google.com/maps?f=d&hl=en&saddr=<?=$coor['lat']?>&daddr=<?=$result['latitude']?>,<?=$result['longitude']?>&sll=&sspn=33.214763,82.265625&z=12">Get Directions</a></div><!--Has Coordinates-->
		<?php }else{ ?>
			<div><a data-map-type="result" target="_blank" href="http://maps.google.com/maps?f=d&hl=en&daddr=<?=$result['latitude']?>,<?=$result['longitude']?>&sll=&sspn=33.214763,82.265625&z=12">Get Directions</a></div><!--No Coordinates-->
		<?php } ?>
	</td>
	<td width="100">
		<div>
